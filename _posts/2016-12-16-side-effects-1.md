...
> Note: цель статьи, обратить внимание на существующую проблему, а не сказать как надо делать или как не надо делать.
...
Хм, теперь кажется что функция не от чего не зависит и вообще ничего не делает. В челом все логично -  не видя реализации мы не можем что-то сказать о том что она делает. Давайте посмотрим на другую функцию:
...
Однако в любом случае составить хороший мок или стаб(и проверить его) зачастую не так то просто. В первую очередь из-за неявности работы с такими объектами - они могут зависеть друг от друга неявно, либо мы просто можем забыть о каком то моке и не протестировать часть функционала.

Чтобы составить хороший мок, нам необходимо просмотреть _весь_ код который исполняется при тестировании определенной сущности. И если для небольшой функции типо `remoteAdd` это не составит труда, то при тестировании всего приложения или его крупной части(некотрого подприложения к примеру) это становится почти не возможно.

Именно это привело к тому что большая часть автоматизированных тестов - это тесты затрагивающие только один unit(модуль), все зависимости которого обычно замоканы. Однако подобные тесты не позволяют выявить ошибки возникшие на стыке модулей - в результате обычной является ситуация когда все тесты проходят, но ничего не работает.
...
Интеграционные же тесты на текущий момент мало распространены, так как они долго запускаются и часто работают не надежно(так как требуется полноценное браузерное окружение) и требует для запуска большого количества ресурсов. Что уничтожает один из главных плюсов автоматизированного тестирования - быстрый отклик и возможность прогонять тесты на каждый коммит.
...
Если мы не контролируем ПО - то оно начинает контролировать нас(разработчиков):
 ...
 - Разработчики зачастую стремятся не переиспользовать крупные части своего приложения(не говоря уже о больших и сложных сторонних компонентах) - так как неясно что они в итоге делают и сможем ли избавится от некотрых нежелательных действий, если они нам вдруг станут не нужны
...
Все наше приложение должно стать полностью чистой функцией: явно принимать все входящие воздействия и явно возвращать выходные. А сайд-эффекты будут исполняться где-то отдельно от основного приложения.
...
Во второй части я попытаюсь вместе с вами реализовать один способов отделения сайд-эффектов от логики.